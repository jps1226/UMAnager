...
public sealed class SeCodeBackfillService
{
    ...
    public async Task<(int Scanned, int Updated)> BackfillAsync(CancellationToken ct = default)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            // (RaceId, HorseId) -> (jockey, trainer). Last record wins per (RaceId, HorseId).
            var bag = new Dictionary<(string, string), (string? j, string? t)>(batch.Count);
            foreach (var row in batch)
            {
                ...
                try
                {
                    ...
                    if (entry.JockeyCode == null && entry.TrainerCode == null) continue;
                    bag[(entry.RaceId, entry.HorseId)] = (entry.JockeyCode, entry.TrainerCode);
                }
    ...
    private static async Task<int> BulkUpdateAsync(
        AppDbContext ctx,
        Dictionary<(string, string), (string? j, string? t)> bag,
        CancellationToken ct)
    {
        ...
        for (int start = 0; start < items.Count; start += ChunkRows)
        {
            ...
            var parameters = new List<object>(chunk.Count * 4);
            int p = 0;

            foreach (var kv in chunk)
            {
                values.Add($"(@p{p}, @p{p+1}, @p{p+2}, @p{p+3})");
            ...
            var sql = $@"
                ...
                SET ""JockeyCode""  = COALESCE(v.j, re.""JockeyCode""),
                    ...
                    ""UpdatedAt""   = NOW()
                FROM (VALUES {string.Join(", ", values)}) AS v(race_id, horse_id, j, t)
